 ...
 */
inline assign_indices_results assign_indices_global(Matrix3d const &A,
    ...
    for (int i = 0; i < rlp.extent(0); ++i) {
        ...
        for (std::size_t j = 0; j < 3; j++) {
            results.miller_indices(i, j) = static_cast<int>(round(hkl_f[j]));
        }
        Vector3d diff{{0, 0, 0}};
        diff[0] = static_cast<double>(results.miller_indices(i, 0)) - hkl_f[0];
        diff[1] = static_cast<double>(results.miller_indices(i, 1)) - hkl_f[1];
        diff[2] = static_cast<double>(results.miller_indices(i, 2)) - hkl_f[2];
        double l_sq = diff.squaredNorm();
        if (l_sq > tolsq) {
            results.miller_indices(i, 0) = 0;
                ...
                for (int i = 0; i < i_same_hkl.size(); i++) {
                    ...
                    for (int j = i + 1; j < i_same_hkl.size(); j++) {
                        ...
                        if (lsq_vector[j_ref] < lsq_vector[i_ref]) {
                            results.miller_indices(i_ref, 0) = 0;
                            results.miller_indices(i_ref, 1) = 0;
                            results.miller_indices(i_ref, 2) = 0;
                            crystal_ids[i_ref] = -1;
                            results.number_indexed--;
                        } else {
                            results.miller_indices(j_ref, 0) = 0;
                            results.miller_indices(j_ref, 1) = 0;
                            results.miller_indices(j_ref, 2) = 0;
                            crystal_ids[j_ref] = -1;
                            results.number_indexed--;
                        }
                    }
